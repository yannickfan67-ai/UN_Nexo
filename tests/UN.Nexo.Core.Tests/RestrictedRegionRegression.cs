...
internal static class RestrictedRegionRegression
{
        ...
        RegionalAccessState expected)
    {
        ...
        if (actual != expected)
            throw new InvalidOperationException(
                $"Regional access state mismatch: expected {expected}, got {actual}.");
    }
    ...
    private static async Task AssertOversizedResponseFailsClosedAsync()
    {
        ...
        if (actual != RegionalAccessState.Unknown)
            throw new InvalidOperationException(
                "Oversized regional trace responses must fail closed.");
    }
    ...
    private static async Task AssertHttpFailureFailsClosedAsync()
    {
        ...
        if (actual != RegionalAccessState.Unknown)
            throw new InvalidOperationException(
                "Regional trace HTTP failures must fail closed.");
    }
    ...
    private static async Task AssertFallbackRequiresVerifiedMicrosoftAsync()
    {
        ...
        var neverVerified = new LauncherAccount(
        ...
        if (await service.CanUseOfflineFallbackAsync(neverVerified))
            throw new InvalidOperationException(
                "A Microsoft profile without a successful entitlement verification must not gain offline fallback.");

        var verified = neverVerified with
        {
            EntitlementVerifiedAt = now
        };
        if (!await service.CanUseOfflineFallbackAsync(verified))
            throw new InvalidOperationException(
                "A previously entitlement-verified Microsoft profile in CN should be eligible for restricted-region fallback.");
