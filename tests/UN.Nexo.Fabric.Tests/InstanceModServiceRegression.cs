...
internal static class InstanceModServiceRegression
{
    ...
    private static async Task RunAsync()
    {
        ...
        try
        {
            ...
            Assert(File.Exists(Path.Combine(paths.GetInstanceGameDirectory("fabric-a"), "mods", "example-mod.jar")),
                "Mod was not copied into the selected instance.");
            ...
            Assert(!disabled.IsEnabled && disabled.FileName == "example-mod.jar.disabled",
                "Disabling should rename the JAR to .jar.disabled.");
            ...
            Assert(await File.ReadAllBytesAsync(Path.Combine(paths.GetInstanceGameDirectory("fabric-a"), "mods", "example-mod.jar")) is var bytes
                   && bytes.SequenceEqual(new byte[] { 9, 8, 7, 6 }),
                "Replacement did not publish the new JAR content.");

            try
            {
                await service.InstallAsync("fabric-a", source, replaceExisting: false);
                throw new Exception("Duplicate install without replacement should fail.");
            }
            catch (IOException)
            {
            }
            ...
            try
            {
                await service.InstallAsync("fabric-a", textFile);
                throw new Exception("Non-JAR mod installation should fail.");
            }
            ...
            try
            {
                service.Remove("fabric-a", "../escape.jar");
                throw new Exception("Traversal mod filename should fail.");
            }
            catch (InvalidDataException)
            {
            }
