...
internal static class InstanceModServiceRegression
{
    internal static async Task RunAsync()
    {
        ...
        try
        {
            ...
            var disabled = service.SetEnabled("fabric-a", installed.FileName, false);
            Assert(!disabled.IsEnabled && disabled.FileName == "example-mod.jar.disabled", "Disabling should rename the JAR to .jar.disabled.");
            Assert(!service.List("fabric-a").Single().IsEnabled, "Disabled state was not listed correctly.");
            await File.WriteAllBytesAsync(source, [9, 8, 7, 6]);
            var replaced = await service.InstallAsync("fabric-a", source, true);
            Assert(replaced.IsEnabled, "Replacing a disabled mod should re-enable the newly installed JAR.");
            Assert(service.List("fabric-a").Count == 1, "Replacing a disabled mod must not leave duplicate entries.");
            ...
            try { await service.InstallAsync("fabric-a", source, false); throw new Exception("Duplicate install without replacement should fail."); } catch (IOException) { }
            ...
            try { await service.InstallAsync("fabric-a", textFile); throw new Exception("Non-JAR mod installation should fail."); } catch (InvalidDataException) { }
            try { service.Remove("fabric-a", "../escape.jar"); throw new Exception("Traversal mod filename should fail."); } catch (InvalidDataException) { }
    ...
    private static async Task VerifyLinkedModsDirectoryIsRejectedAsync(NexoPathService paths, InstanceModService service, string source, string root)
    {
        ...
        try { await service.InstallAsync("fabric-linked", source); throw new Exception("Installing through a linked mods directory should fail."); } catch (InvalidDataException) { }
        Assert(!File.Exists(Path.Combine(outside, Path.GetFileName(source))), "Linked mods install wrote outside the instance.");
        try { service.List("fabric-linked"); throw new Exception("Listing through a linked mods directory should fail closed."); } catch (InvalidDataException) { }
    }
