...
## Authenticated launch requirement

UN_Nexo does not provide an offline Minecraft launch mode. Packaged builds require a Microsoft profile that can complete the normal Microsoft -> Xbox Live -> XSTS -> Minecraft Services flow and produce a valid Minecraft Java profile and launch token.

Network failures, regional service availability problems, expired credentials, entitlement failures, and Minecraft AppID authorization failures do not fall back to an offline identity. The launcher reports the authentication/service error and does not synthesize or substitute an unauthenticated Minecraft session.
