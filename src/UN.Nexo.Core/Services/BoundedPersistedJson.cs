...
internal static class BoundedPersistedJson
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using var bounded = new BoundedReadStream(file, MaxStoreBytes, Path.GetFileName(path));
        return await JsonSerializer.DeserializeAsync<T>(bounded, options, cancellationToken);
    }

    private static InvalidDataException Oversized(string path) =>
        new($"Persisted store '{Path.GetFileName(path)}' exceeds the {MaxStoreBytes} byte safety limit. The original file was preserved.");
    ...
    private sealed class BoundedReadStream : Stream
    {
        ...
        internal BoundedReadStream(Stream inner, long maxBytes, string name)
        {
            _inner = inner;
            _maxBytes = maxBytes;
            _name = name;
        ...
        public override async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default)
        {
            ...
            _bytesRead += read;
            await ProbeOverflowIfAtLimitAsync(read, cancellationToken);
            return read;
        }
        ...
        private int GetAllowedCount(int requested)
        {
            ...
            return (int)Math.Min(requested, _maxBytes - _bytesRead);
        }
        ...
        private async ValueTask ProbeOverflowIfAtLimitAsync(int read, CancellationToken cancellationToken)
        {
            if (read == 0 || _bytesRead < _maxBytes)
                return;
            var probe = new byte[1];
            if (await _inner.ReadAsync(probe.AsMemory(), cancellationToken) != 0)
                throw Oversized(_name);
        }
