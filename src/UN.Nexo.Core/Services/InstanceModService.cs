...
public sealed class InstanceModService
{
    ...
    private string? VerifyModsDirectory(string instanceId, bool create)
    {
        ...
        RejectLinkIfPresent(instanceDirectory);
        RejectLinkIfPresent(gameDirectory);
        RejectLinkIfPresent(modsDirectory);
    ...
    {
        if (Directory.Exists(path) || File.Exists(path)) VerifyPhysicalDirectory(path);
    }
    ...
    private static InstalledMod CreateModel(string path)
    {
        ...
        return new InstalledMod(fileName, fileName.EndsWith(".jar", StringComparison.OrdinalIgnoreCase), info.Length, info.LastWriteTimeUtc);
    }

    private static bool IsManagedModFileName(string fileName)
        => fileName.EndsWith(".jar", StringComparison.OrdinalIgnoreCase) || fileName.EndsWith(".jar" + DisabledSuffix, StringComparison.OrdinalIgnoreCase);

    private static void ValidateManagedFileName(string fileName, bool allowDisabled)
    {
        if (string.IsNullOrWhiteSpace(fileName) || fileName is "." or ".." || !string.Equals(Path.GetFileName(fileName), fileName, StringComparison.Ordinal) || fileName.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)
