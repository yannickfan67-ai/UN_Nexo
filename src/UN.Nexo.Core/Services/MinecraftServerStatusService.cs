...
public sealed class MinecraftServerStatusService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var offset = 0;
            var packetId = ReadVarInt(responsePayload, ref offset);
            if (packetId != 0)
            ...
            try
            {
                ...
                await WritePacketAsync(stream, packet, token);
                await stream.FlushAsync(token);

                var stopwatch = Stopwatch.StartNew();
                var pongPayload = await ReadPacketAsync(stream, token);
                stopwatch.Stop();
                var pongOffset = 0;
                if (ReadVarInt(pongPayload, ref pongOffset) == 1
                    && pongOffset + sizeof(long) <= pongPayload.Length)
                {
                    _ = BinaryPrimitives.ReadInt64BigEndian(pongPayload.AsSpan(pongOffset, sizeof(long)));
                    latency = stopwatch.ElapsedMilliseconds;
                }
            }
            catch (Exception ex) when (ex is IOException or SocketException or InvalidDataException)
            {
                // A valid status response is still useful if the optional ping exchange fails.
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        BinaryPrimitives.WriteUInt16BigEndian(port, checked((ushort)target.Port));
        payload.Write(port);
        WriteVarInt(payload, 1);
    ...
    private static string ReadString(ReadOnlySpan<byte> buffer, ref int offset)
    {
        ...
        if (byteLength is < 0 or > MaxStringBytes || offset + byteLength > buffer.Length)
            throw new InvalidDataException("Server returned an invalid status string length.");
        var value = Encoding.UTF8.GetString(buffer.Slice(offset, byteLength));
        offset += byteLength;
        return value;
    }
    ...
    private static ServerStatusResult ParseStatus(string json, string address)
    {
        ...
        if (root.TryGetProperty("version", out var version) && version.ValueKind == JsonValueKind.Object)
        {
            ...
            if (version.TryGetProperty("protocol", out var protocolElement) && protocolElement.TryGetInt32(out var protocolValue))
        ...
        {
            if (players.TryGetProperty("online", out var online) && online.TryGetInt32(out var onlineValue))
                onlinePlayers = onlineValue;
            if (players.TryGetProperty("max", out var max) && max.TryGetInt32(out var maxValue))
                maxPlayers = maxValue;
        }

        var motd = root.TryGetProperty("description", out var description)
            ? FlattenText(description)
            : string.Empty;
        ...
        switch (element.ValueKind)
        {
            ...
            case JsonValueKind.Object:
            {
                ...
                if (element.TryGetProperty("text", out var text))
                    builder.Append(FlattenText(text));
                if (element.TryGetProperty("extra", out var extra))
                    builder.Append(FlattenText(extra));
                if (builder.Length == 0 && element.TryGetProperty("translate", out var translate))
                    builder.Append(FlattenText(translate));
