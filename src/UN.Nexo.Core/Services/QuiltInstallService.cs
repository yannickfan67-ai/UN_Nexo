...
public sealed class QuiltInstallService(
    ...
    TimeSpan? transferIdleTimeout = null)
{
    ...
    private readonly TimeSpan _transferIdleTimeout = transferIdleTimeout is null
        ...
        : transferIdleTimeout.Value > TimeSpan.Zero
            ...
            : throw new ArgumentOutOfRangeException(
                ...
                "Quilt transfer idle timeout must be positive.");
        ...
        bool acquireOperationLease)
    {
        if (!instance.Loader.Equals("quilt", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("Quilt preparation requires a Quilt instance.");
        ...
        if (profile is null)
        {
            if (string.IsNullOrWhiteSpace(instance.LoaderVersion))
                throw new InvalidOperationException(
                    "Quilt Loader version is missing and no imported Quilt profile is available.");
            profile = await quiltMeta.GetProfileAsync(
                baseVersion.Id,
        ...
        using (profile)
        {
            ...
            try
            {
                ...
                var completed = 0;
                Report(progress, new InstallProgress("Quilt libraries", 0, libraries.Count));
                ...
                Report(progress, new InstallProgress(
                    ...
                    1,
                    instance.VersionId,
                    Detail: $"Quilt Loader {loaderVersion} is ready"));
            }
            catch
            {
                InstallStateSnapshot.Delete(
    ...
    public static string? ReadLoaderVersion(JsonElement profile)
    {
        ...
        foreach (var library in libraries.EnumerateArray())
        {
            ...
            const string prefix = "org.quiltmc:quilt-loader:";
        ...
        string expectedBaseVersion)
    {
        ...
        if (!string.Equals(profileId, expectedId, StringComparison.Ordinal))
            throw new InvalidDataException(
                $"Quilt profile id '{profileId}' does not match instance version '{expectedId}'.");

        var inherited = RequireString(profile, "inheritsFrom");
        if (!string.Equals(inherited, expectedBaseVersion, StringComparison.Ordinal))
            throw new InvalidDataException(
                $"Quilt profile inherits '{inherited}', expected Minecraft {expectedBaseVersion}.");
    ...
    private static InvalidDataException InvalidProfile(string propertyName, string expected)
        => new($"Quilt profile property '{propertyName}' must be {expected}.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!await IsValidAsync(temp, expectedSha1, cancellationToken))
                throw new InvalidDataException(
                    $"Downloaded Quilt library failed verification: {library.Url}");
        ...
        CancellationToken cancellationToken)
    {
        using var response = await TrustedHttpDownload.SendGetAsync(
            httpClient,
            url,
            "Quilt library checksum",
            cancellationToken);
        response.EnsureSuccessStatusCode();

        if (response.Content.Headers.ContentLength is > MaxChecksumBytes)
            throw new InvalidDataException(
                $"Quilt checksum response is too large: {url}");
        ...
        if (token is null
            ...
            throw new InvalidDataException(
                $"Quilt checksum response is invalid: {url}");
        ...
        while (true)
        {
            ...
            catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
            {
                throw new TimeoutException(
                    $"Quilt transfer from {source} made no progress for {_transferIdleTimeout.TotalSeconds:0.###} seconds.");
            }
            ...
            if (maxBytes is not null && total > maxBytes.Value)
                throw new InvalidDataException(
                    $"Quilt metadata transfer from {source} exceeded the {maxBytes.Value}-byte limit.");
