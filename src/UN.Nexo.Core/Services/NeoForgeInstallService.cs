...
public sealed class NeoForgeInstallService
{
    ...
    private readonly Func<
        ProcessStartInfo,
        CancellationToken,
        Task<NeoForgeInstallerProcessResult>> _processRunner;

    public NeoForgeInstallService(
        ...
        Func<
            ProcessStartInfo,
            CancellationToken,
            Task<NeoForgeInstallerProcessResult>>? processRunner = null,
            ...
            Task<JavaInstallation>>? javaProvisioner = null)
    {
        ...
        if (_transferIdleTimeout <= TimeSpan.Zero)
        {
            throw new ArgumentOutOfRangeException(
                ...
                "NeoForge transfer idle timeout must be positive.");
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                "NeoForge preparation requires a NeoForge instance.");
        }

        if (string.IsNullOrWhiteSpace(instance.LoaderVersion))
            throw new InvalidOperationException(
                "NeoForge version is missing from the instance metadata.");
        var neoforgeVersion = instance.LoaderVersion.Trim();
                ...
                StringComparison.Ordinal))
        {
            throw new InvalidDataException(
                $"NeoForge instance expects Minecraft {instance.BaseVersionId}, not {baseVersion.Id}.");
        }

        var expectedLaunchId = NeoForgeMetaService.GetLaunchVersionId(
            baseVersion.Id,
            neoforgeVersion);
        ...
        {
            throw new InvalidDataException(
                $"NeoForge launch profile '{instance.VersionId}' does not match expected '{expectedLaunchId}'.");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
                           ...
                           cancellationToken))
            {
                ...
                {
                    throw new InvalidDataException(
                        $"NeoForge profile resolved client '{resolved.ClientVersionId}', expected '{baseVersion.Id}'.");
                }
            }
            ...
            var state = new
            {
                ...
                source = "neoforged-official-installer",
                state = "prepared"
            };
            ...
            Report(
                ...
                new InstallProgress(
                    ...
                    instance.VersionId,
                    Detail:
                        $"NeoForge {neoforgeVersion} is ready"));
        }
        catch
        {
            TryDeleteRegularFile(statePath);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        RejectExistingReparseFile(
            profilePath,
            "NeoForge launch metadata");
        var info = new FileInfo(profilePath);
        if (info.Length > 8L * 1024L * 1024L)
        {
            throw new InvalidDataException(
                "NeoForge launch metadata exceeds the 8 MiB local metadata limit.");
        }
        ...
        if (root.ValueKind != JsonValueKind.Object)
            throw new InvalidDataException(
                "NeoForge launch metadata root must be an object.");
        ...
        {
            throw new InvalidDataException(
                "NeoForge launch metadata property 'inheritsFrom' must be a non-empty string.");
        }
    ...
    {
        var installerUrl =
            NeoForgeMetaService.GetInstallerUrl(
                minecraftVersion,
                neoforgeVersion);
        var expectedSha1 =
        ...
        var neoforgeRoot =
            EnsurePhysicalChildDirectory(
                cacheRoot,
                "neoforge",
                "NeoForge installer cache");

        var fileName =
            $"neoforge-{neoforgeVersion}-installer.jar";
        var target =
            Path.Combine(neoforgeRoot, fileName);
        ...
        try
        {
            ...
            {
                throw new InvalidDataException(
                    $"NeoForge installer exceeds the {MaxInstallerBytes}-byte limit.");
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException(
                "NeoForge installer checksum response is too large.");
        }
        ...
        {
            throw new InvalidDataException(
                "NeoForge Maven returned an invalid installer SHA-1.");
        }
        ...
        string launchVersionId)
    {
        ...
        {
            throw new InvalidDataException(
                $"NeoForge installer completed but did not create {launchVersionId}.json.");
        }
    ...
    private static async Task<NeoForgeInstallerProcessResult>
        RunProcessAsync(
    ...
    private static async Task WriteInstallerLogAsync(
        string instanceRoot,
        ProcessStartInfo startInfo,
        NeoForgeInstallerProcessResult result,
        CancellationToken cancellationToken)
    {
        var logs =
            EnsurePhysicalChildDirectory(
